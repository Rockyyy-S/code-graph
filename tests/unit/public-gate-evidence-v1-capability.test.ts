import { expect, it } from "vitest";
import { validateGateEvidenceV1 } from "../../packages/contracts/src/index.js";
import { runBoundPublicCapabilityTest } from "../../scripts/contracts/run-public-capability-verification.mjs";

it("验证 GateEvidenceV1 公共能力的正负向合同", async () => {
  if (process.env.CODEGRAPH_PUBLIC_CAPABILITY !== "schema:gateEvidenceV1Schema") {
    return;
  }
  await runBoundPublicCapabilityTest({
    capabilityId: "schema:gateEvidenceV1Schema",
    evidenceId: "public-capability:schema:gateEvidenceV1Schema",
    fixturePath: "tests/fixtures/public-gate-evidence-v1.json",
    verifyNegative: async ({ fixture }: { fixture: Record<string, unknown> }) => {
      expect(validateGateEvidenceV1({ ...fixture, unknown: true })).toBe(false);
    },
    verifyPositive: async ({ fixture }: { fixture: Record<string, unknown> }) => {
      expect(validateGateEvidenceV1(fixture)).toBe(true);
    },
  });
});
